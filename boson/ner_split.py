            ...
            if line[i] == '{':      # 发现实体
                ...
                if len(entity)==1:  # 针对S标记
                    ...
                    for j in range(1, len(entity)-1):
                        tagged_line_str += entity[j]+'/I_'+tag+' '
...
def main():
    ...
    # 按8:2切分数据集为训练集、测试集
    train_list, test_list = train_test_split(sentences_list, shuffle=True, ratio=0.8)

    # 将实体标注转换为BIOES标注
    tagged_train_list, tagged_test_list = origin2tag(train_list), origin2tag(test_list)

    # 按照”字 tag\n“的格式，平铺所有数据,并保存
    with open('./result/bosonner.train.bioes', 'w', encoding='utf-8') as train_file, open('./result/bosonner.test.bioes', 'w', encoding='utf-8') as test_file:
